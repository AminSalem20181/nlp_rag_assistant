...
# # -----------------------------
# st.set_page_config(page_title="GapGPT RAG Pro", layout="wide")

# # -----------------------------
# # ۲. لود مدل‌ها (Cache شده)
# # -----------------------------
# @st.cache_resource
# def load_resources():
#     # مدل آفلاین
...
# # سایدبار برای مدیریت چت‌ها و تنظیمات مدل
# with st.sidebar:
#     st.title("⚙️ Settings")
    
#     # مدیریت چت‌ها
#     new_chat_name = st.text_input("New Chat Name", placeholder="Enter name...")
...
#     chat_selection = st.selectbox("Your Chats", options=list(st.session_state.chats.keys()), index=list(st.session_state.chats.keys()).index(st.session_state.current_chat))
#     st.session_state.current_chat = chat_selection

#     st.divider()
    
#     # انتخاب حالت مدل (Mode Selection)
#     st.subheader("🤖 Model Mode")
#     app_mode = st.radio(
#         "Select Operation Mode:",
#         ["Compture Knowledge", "General Knowledge", "Hybrid (Compture + General)"]
#     )
#     st.info(f"Current Mode: {app_mode}")
...
#         context_list.append(raw_text)
#         sources.append(clean_text)
    
#     context_text = "\n\n".join(context_list)
    
#     if app_mode == "Compture Knowledge":
#         prompt = f"Answer strictly using context: {context_text}\n\nQuestion: {user_input}\nAnswer:"
#     elif app_mode == "Hybrid (Compture + General)":
#         prompt = f"Use context if helpful: {context_text}\n\nQuestion: {user_input}\nAnswer:"
#     else: # Online Mode
#         prompt = f"Question: {user_input}\nAnswer:"

#     inputs = tokenizer(prompt, return_tensors="pt", truncation=True, max_length=512)
#     outputs = llm_model.generate(**inputs, max_new_tokens=150, do_sample=False)
...
# # نمایش تاریخچه چت انتخاب شده
# for msg in st.session_state.chats[st.session_state.current_chat]:
#     with st.chat_message(msg["role"]):
#         st.markdown(msg["content"])
#         if "sources" in msg:
#             with st.expander("📚 Offline Sources"):
...
#             ans, srcs = get_offline_response(prompt)
#             st.markdown(ans)
